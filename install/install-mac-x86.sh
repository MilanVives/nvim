...
# Neovim Installation Script for Mac x86_64
# Downloads and installs Neovim for Intel Macs
...
# Verify download
if [[ ! -f "nvim-macos-x86_64.tar.gz" ]]; then
...
# Verify extraction
if [[ ! -f "nvim-macos-x86_64/bin/nvim" ]]; then
...
sudo rm -rf "$INSTALL_PREFIX/nvim-macos-x86_64"

# Move to installation directory
log_info "Installing Neovim..."
sudo mv nvim-macos-x86_64 "$INSTALL_PREFIX/"

# Create symlink
log_info "Creating symlink..."
sudo ln -sf "$INSTALL_PREFIX/nvim-macos-x86_64/bin/nvim" "$INSTALL_PREFIX/bin/nvim"
...
if "$INSTALL_PREFIX/nvim-macos-x86_64/bin/nvim" --version &> /dev/null; then
    installed_version=$("$INSTALL_PREFIX/nvim-macos-x86_64/bin/nvim" --version | head -n1)
